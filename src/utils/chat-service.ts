 ...
 */
async function generateEmbeddingViaEdgeFunction(
  query: string,
  clientId: string,
): Promise<number[]> {
  try {
    console.log("chat-service: calling embedding edge function", {
    ...
    console.error("chat-service: embedding generation failed", {
...
export const sendChatMessageToGrok = async ({
  ...
  try {
    ...
    if (
      settingsError || !settings?.groq_api_key || !settings?.groq_model_name
    ) {
    ...
    if (!userLatestMessage) {
      throw new Error(
        "Unable to find the latest user message in the conversation history.",
      );
    }

    // 4. Generate 768-dim Embedding Vector
    const userQueryVector = await generateEmbeddingViaEdgeFunction(
      userLatestMessage,
      clientId,
    );
...
If the answer cannot be found in the context, state clearly: "I cannot find this information in the course knowledge base."`;
    ...
    if (!currentSessionId) {
      ...
      if (sessionInsertError || !newSession) {
        console.error(
          "Database transactional rollback fallback: Failed to create thread record.",
          sessionInsertError,
        );
        throw new Error(
          `Failed to initialize chat log record: ${sessionInsertError?.message}`,
        );
      }
    ...
    if (messagesError) {
      console.error(
        "Database isolation rollback trace: Message logging failure",
        messagesError,
      );
      ...
      throw new Error(
        `Failed to save message entries to chat logs: ${messagesError.message}`,
      );
    }
    ...
    console.error("chat-service: execution routine exception", {
      error: getErrorMessage(error),
    });
    throw error;
  }
